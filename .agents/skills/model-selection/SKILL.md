...
python3 SKILL_DIR/pick-models.py \
  ...
  --prefer-provider anthropic --prefer-provider openai --prefer-provider google \
...
Preference helpers:
- `--prefer-provider`: e.g., `anthropic`, `openai`, `google` (optionally `openai-codex`)
- `--prefer-company`: e.g., `openai`, `anthropic`, `google`
- `--prefer-model`: substring hints, e.g., `codex`, `5.4`
- `--avoid-provider`: hard avoid, e.g., `github-copilot`

```bash
# Example (typical default policy)
python3 SKILL_DIR/pick-models.py --role coder --role reviewer --role scout --prefer-provider anthropic --prefer-provider openai --prefer-provider google --prefer-model codex --avoid-provider github-copilot
```
