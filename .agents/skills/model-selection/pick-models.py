...
def filter_avoided_providers(roster, avoided_providers):
    """Hard-avoid providers (no fallback)."""
    avoided = set(avoided_providers or [])
    if not avoided:
        return roster

    return [m for m in roster if (m.get("provider") or "").lower() not in avoided]
...
def score_with_preferences(base_score, model, preferences, generation_rank=None):
    ...
    # If Gemini is available from direct Google provider, prefer it over routed variants.
    if "gemini" in model_text and provider == "google":
        score += 120.0
    elif "gemini" in model_text and provider == "github-copilot":
        score -= 80.0
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Hard-avoid provider (can repeat or use commas)",
    )
...
def main():
    ...
    if not filtered:
        avoided = preferences.get("avoid_providers") or []
        if avoided:
            print(
                "No models remain after hard avoid-provider filtering. "
                "Adjust --avoid-provider or add non-avoided providers.",
                file=sys.stderr,
            )
            sys.exit(2)
        ...
        if blocked:
            print(
                "No models remain after 429 filtering. Clear expired blocks or wait cooldown.",
                file=sys.stderr,
            )
            sys.exit(2)
