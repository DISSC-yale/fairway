...
PROVIDER_PRIORITY = {
    ...
    "openai": 95,
    "anthropic": 90,
    "google-gemini-cli": 85,
    "google": 80,
    "xai": 70,
    "z.ai": 60,
    "google-antigravity": 15,
    "github-copilot": 10,
}
...
def match_or_slug(pi_provider, pi_model, pricing):
    ...
    provider_map = {
        ...
        "google-antigravity": None,   # mixed: gemini + claude + gpt-oss
        "github-copilot": None,
    }
    ...
    if or_provider is None:
        # Multi-model providers (github-copilot, google-antigravity, etc.)
        # — try all known companies as OpenRouter prefix.
        for p in COMPANIES:
...
def build_roster_from_pi(pi_models, pricing, meta):
    ...
    for pm in pi_models:
        ...
        # For throughput, map wrapper providers to their real provider's permaslug
        real_slug = None
        if provider_priority(pm["pi_provider"]) < provider_priority("google"):
            # Low-priority / wrapper providers (github-copilot, google-antigravity, etc.)
