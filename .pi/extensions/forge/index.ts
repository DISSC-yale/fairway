 ...
 *   - research: Parallel multi-source investigation with synthesis
 *   - /ship: Ship issues through scout→plan→code→review pipeline
 *   - /plan: Autonomous planning loop (draft→review→fix until convergence)
 */
...
export default function (pi: ExtensionAPI) {
	...
	registerResearch(pi, currentDepth);
	registerShip(pi);
	registerPlan(pi, currentDepth);
