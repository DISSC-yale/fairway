...
const DEFAULT_PREFERENCE_PROFILE: ModelSelectionPreferences = {
	...
	preferredCompanies: ["anthropic", "openai", "google"],
...
function filterAvoidedProviders(models: ParsedModel[], avoidProviders?: string[]): ParsedModel[] {
	...
	return models.filter((m) => !avoided.has(m.provider));
}
...
function preferenceBonus(model: ParsedModel, preferences?: ModelSelectionPreferences): number {
	...
	if (text.includes("gemini") && provider === "google") bonus += 6;
	if (text.includes("gemini") && provider === "github-copilot") bonus -= 8;
...
function recommend(
	...
	// Filter to providers that actually have credentials configured.
	// Fall back to full pool if filtering yields nothing (safety net).
	const live = availableProviders();
	const livePool = preferredPool.filter((m) => live.includes(m.provider));
	const pool = livePool.length > 0 ? livePool : preferredPool;

	const generationRank = buildGenerationRank(pool);
...
export function registerModels(pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Default preference policy: prefer Anthropic/OpenAI/direct Google providers, favor OpenAI Codex variants, hard-avoid github-copilot unless explicitly requested.",
			...
			"Prefer different providers/companies in debates for diverse perspectives. If only one provider is configured, use same-provider agents with different model tiers and distinct personas.",
		...
		async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
			...
			if (pick) {
				...
				if (usedDefaults) {
					preferenceLines.push("  - preference profile: default (anthropic/openai/google preferred, codex favored, github-copilot hard-avoided)");
				} else {
			...
			if (recommended.length === 0) {
				throw new Error("No models remain after applying provider-avoid preferences. Remove/adjust avoidProviders or add additional providers.");
			}
			...
			const text = [
				...
				"",
				`### Recommended (${new Set(recommended.map(r => r.provider)).size > 1 ? "diverse providers" : "available models"})`,
				"",
			];
