...
export async function runAgent(opts: RunAgentOptions): Promise<DispatchResult> {
	...
	try {
		...
		const envStall = Number(process.env.FORGE_STALL_TIMEOUT_MS) || 120_000;
		const STALL_MS = opts.stallMs !== undefined ? opts.stallMs : envStall;
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			const effectiveTimeout = opts.timeoutMs ?? 600_000;
			if (effectiveTimeout > 0) {
				const deadlineTimer = setTimeout(() => {
					wasAborted = true;
					abortReason = `wall-clock timeout after ${Math.round(effectiveTimeout / 1000)}s`;
					proc.kill("SIGTERM");
					setTimeout(() => {
						if (!processExited) proc.kill("SIGKILL");
					}, 5000);
				}, effectiveTimeout);
				proc.on("close", () => clearTimeout(deadlineTimer));
			}

			// Stall detection: kill child if no stdout events for STALL_MS
			const stallCheck = STALL_MS > 0 ? setInterval(() => {
				if (!processExited && Date.now() - lastEventAt > STALL_MS) {
					clearInterval(stallCheck!);
					wasAborted = true;
					result.stopReason = "error";
					result.errorMessage = `Stalled: no output for ${Math.round(STALL_MS / 1000)}s`;
					proc.kill("SIGTERM");
					setTimeout(() => { if (!processExited) proc.kill("SIGKILL"); }, 5000);
				}
			}, 15_000) : null;
			proc.on("close", () => { if (stallCheck) clearInterval(stallCheck); });
		});

		result.exitCode = exitCode;
		if (wasAborted) throw new Error(`Agent aborted: ${abortReason}`);
		return result;
	} finally {
...
export function registerDispatch(pi: ExtensionAPI, currentDepth: number) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Agents with ['read','bash'] tools can search the web (curl websearch.kyle.pub), scrape pages (curl_chrome142), search GitHub (gh search), and clone repos. Mention relevant skills in personas so agents know: .agents/skills/web-search/SKILL.md, .agents/skills/web-scrape/SKILL.md, .agents/skills/github-issues/SKILL.md.",
