...
function formatEntry(e: LogEntry): string {
	...
	if (e.tool === "dispatch") {
		...
		const p = e.params;
		md += `- **objective**: ${truncate(String(p.objective || ""), 200)}\n`;
		const cfg = p.config as Record<string, unknown> | undefined;
		if (cfg) {
			md += `- **drafter**: ${cfg.drafter || "(default)"}\n`;
			md += `- **reviewers**: ${Array.isArray(cfg.reviewers) ? (cfg.reviewers as string[]).join(", ") : "(default)"}\n`;
			md += `- **maxCycles**: ${cfg.maxCycles || 4}\n`;
			md += `- **scout**: ${cfg.scout ?? true}\n`;
		}
