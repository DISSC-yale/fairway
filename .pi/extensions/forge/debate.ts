...
export function registerDebate(pi: ExtensionAPI, currentDepth: number) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Prefer models from different providers in debates for diverse perspectives. If only one provider is available, use different model tiers (e.g., opus + sonnet) with distinct personas.",
			"Debate agents with ['read','bash'] tools can search the web, scrape pages, and search GitHub if they need to verify a claim or check external docs. Mention .agents/skills/web-search/SKILL.md in personas when relevant.",
