...
class AgentEndClearedError extends Error {
	constructor(reason?: string) {
		super(reason ?? "agent_end waiter cleared");
		this.name = "AgentEndClearedError";
	}
}
...
let rejectAgentDone: ((reason?: unknown) => void) | null = null;
const SHIP_WAIT_TIMEOUT_MS = Number(process.env.FORGE_SHIP_WAIT_TIMEOUT_MS) || 3_600_000;
const FOLLOWUP_TIMEOUT_MS = Number(process.env.FORGE_SHIP_FOLLOWUP_TIMEOUT_MS) || 180_000;
const COMPACT_TIMEOUT_MS = Number(process.env.FORGE_COMPACT_TIMEOUT_MS) || 45_000;
const STALE_RUN_TIMEOUT_MS = Math.max(
	Number(process.env.FORGE_STALE_RUN_TIMEOUT_MS) || 360_000,
	SHIP_WAIT_TIMEOUT_MS + 60_000,
);

/** Called from the single agent_end listener. */
function signalAgentEnd(): void {
	if (activeRun) touchRunActivity(activeRun);
	if (resolveAgentDone) {
		const r = resolveAgentDone;
		resolveAgentDone = null;
		rejectAgentDone = null;
		r();
	}
}

/** Discard a pending waiter without resolving. For cleanup only. */
function clearAgentEndWaiter(reason?: string): void {
	if (rejectAgentDone) {
		const r = rejectAgentDone;
		rejectAgentDone = null;
		resolveAgentDone = null;
		r(new AgentEndClearedError(reason));
	}
	resolveAgentDone = null;
}
...
function nextAgentEnd(): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		rejectAgentDone = reject;
	});
}

// ── Compaction helper ──────────────────────────────────────────────

/** Promise wrapper around ctx.compact() — fail-open on error, with timeout. */
function compactAndWait(ctx: ExtensionCommandContext, instructions: string): Promise<void> {
	...
	return Promise.race([
		...
		new Promise<void>((_, reject) => {
			timer = setTimeout(() => reject(new Error("compaction timed out")), COMPACT_TIMEOUT_MS);
		}),
...
function armRunWatchdog(run: ShipRun, ctx: ExtensionCommandContext): void {
	run.lastActivityAt = Date.now();
	run.watchdog = setInterval(() => {
		const idleMs = Date.now() - run.lastActivityAt;
		if (idleMs < STALE_RUN_TIMEOUT_MS) return;
		run.cancelled = true;
		clearAgentEndWaiter("watchdog: stale run");
		if (activeRun?.id === run.id) activeRun = null;
		clearInterval(run.watchdog);
		if (ctx.hasUI) ctx.ui.notify("/ship watchdog cleared a stale run after prolonged inactivity.", "error");
	}, 15_000);
}

function touchRunActivity(run: ShipRun): void {
	run.lastActivityAt = Date.now();
}

// ── Prompt builders ────────────────────────────────────────────────

const TOOL_RECOVERY_GUIDANCE = "If a dispatch or debate fails (stall, timeout, or error), try a different model or fewer agents — don't retry the same call unchanged.";
const DEBATE_FIX_LOOP = "After each debate, if reviewers did not reach LGTM: read the round files, apply their feedback to the plan or code, then re-run the debate. Do not re-run a debate without fixing what was flagged. This is the core loop: review → fix → review.";
const RESEARCH_GUIDANCE = "You have a research() tool — use it whenever you need external information (benchmarks, docs, community reports, code examples, prior art). Staff a team of researchers with specific angles and tools (['read','bash'] covers web search, scraping, gh/fj, git clone). For pure research issues (spikes), the deliverable is a summary posted as an issue comment — skip plan/code/review phases.";
...
function singleIssuePrompt(issue: number): string {
	return [
		...
		"Then: run `git remote get-url origin` to detect the forge — use `gh` for github.com, `fj` for Forgejo/Gitea, or `curl` API as fallback. Fetch the issue with the detected tool.",
		...
		RESEARCH_GUIDANCE,
		"",
		"For non-spike issues: required gates are mandatory scout; plan debate to LGTM; code; review debate to LGTM; if review is not consensus LGTM, return to coding. Do not ship until both `plan-review/consensus.md` and `code-review/consensus.md` exist in the issue workDir.",
		"",
		DEBATE_FIX_LOOP,
		"",
		TOOL_RECOVERY_GUIDANCE,
	].join("\n");
}
...
function sequenceIssuePrompt(issue: number, index: number, total: number, remaining: number[]): string {
	const lines = [
		...
		"2. Run `git remote get-url origin` to detect the forge — use `gh` for github.com, `fj` for Forgejo/Gitea, or `curl` API as fallback. Fetch the issue.",
		`3. \`rm -rf /tmp/forge/issue-${issue}/ && mkdir -p /tmp/forge/issue-${issue}/\``,
		`4. Follow the forge /ship workflow strictly. Every dispatch and debate MUST set workDir under \`/tmp/forge/issue-${issue}/\`.`,
		"",
		RESEARCH_GUIDANCE,
		"",
		"4. For non-spike issues: required gates are mandatory scout; plan debate to LGTM; code; review debate to LGTM; if review is not consensus LGTM, return to coding. Do not ship until both `plan-review/consensus.md` and `code-review/consensus.md` exist.",
		"",
...
function localTaskPrompt(source: string): string {
	...
	return [
		...
		RESEARCH_GUIDANCE,
		"",
		"For non-spike tasks: required gates are mandatory scout; plan debate to LGTM; code; review debate to LGTM. Do not ship until both `plan-review/consensus.md` and `code-review/consensus.md` exist.",
...
async function enforceConsensusGates(
	...
	for (let attempt = 1; attempt <= maxNudges; attempt++) {
		...
		done.catch(() => {}); // rejection sink: guards sync-throw window before race
		pi.sendUserMessage(prompt, { deliverAs: "followUp" });
		const result = await Promise.race([
			done.then(() => "done" as const),
			new Promise<"timeout">((resolve) =>
				setTimeout(() => resolve("timeout"), FOLLOWUP_TIMEOUT_MS),
			),
		]);
		if (result === "timeout") {
			clearAgentEndWaiter("consensus nudge deadline exceeded");
			return false;
		}
	}
	...
	pi.registerCommand("ship", {
		...
		handler: async (args, ctx) => {
			...
			if (target.kind === "local") {
				...
				const run: ShipRun = { id: nextRunId++, cancelled: false, lastActivityAt: Date.now() };
				activeRun = run;
				armRunWatchdog(run, ctx);
				...
				try {
					...
					const result = await Promise.race([
						...
						new Promise<"timeout">((resolve) =>
							setTimeout(() => resolve("timeout"), SHIP_WAIT_TIMEOUT_MS),
						),
					]);
					if (result === "timeout") {
						clearAgentEndWaiter("deadline exceeded");
						if (ctx.hasUI) ctx.ui.notify("Ship timed out waiting for agent. Hard fail.", "error");
						return;
					}
					...
					if (!(e instanceof AgentEndClearedError)) throw e;
					// watchdog or cancellation — cleanup handled in finally
				} finally {
					clearAgentEndWaiter();
